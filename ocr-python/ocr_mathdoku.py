...
def _is_valid_cage_value(
    ...
    # No operator detected — value must be valid for at least one operator.
    if n_cells == 1:
        return 1 <= value <= grid_size
    possible_ops = ["+", "x"]
    if n_cells == 2:
        possible_ops.extend(["-", "/"])
    return any(
        _is_valid_cage_value(value_str, test_op, n_cells, grid_size)
        for test_op in possible_ops
    )
