...
import { TooBigInSumStrategy } from '../../src/strategies/TooBigInSumStrategy.ts';
...
describe('TooBigInSumStrategy', () => {
  const strategy = new TooBigInSumStrategy();
  ...
  it('eliminates candidates that are too big for the cage sum', () => {
    ...
    expect(note).toContain('Too big in sum.');
  });
